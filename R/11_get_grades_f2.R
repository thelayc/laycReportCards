...
get_grades_f2 <- function(student_rcard){
  ...
  chunk_start <- which(stringr::str_detect(student_rcard, 'Term progress') == TRUE)
...
#   #final_position <- stringr::str_locate(term, 'Final')[1]
  ...
  #2) Identify position of each grade on the line
  term <- student_rcard[chunk_start + 1]
  # Locate the starting position of "Term 1" in the string - First occurrence of TERM
  g1_position <- stringr::str_locate(term, '1')[1]
  g2_position <- stringr::str_locate(term, '2')[1]
  g3_position <- stringr::str_locate(term, '3')[1]
  g4_position <- stringr::str_locate(term, '4')[1]
  ...
  term4 <- stringr::str_trim(substr(grade_lines, g4_position, g4_position + 1))
